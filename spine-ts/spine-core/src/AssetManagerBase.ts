...
export class AssetManagerBase implements Disposable {
  ...
  constructor(
    textureLoader: (image: HTMLImageElement | ImageBitmap) => Texture,
    pathPrefix: string = "",
...
export class Downloader {
  ...
  dataUriToString(dataUri: string) {
    ...
    if (base64Idx != -1) {
      ...
      return atob(dataUri.substring(base64Idx))
    } else {
  ...
  dataUriToUint8Array(dataUri: string) {
    ...
    base64Idx += "base64,".length
    return this.base64ToUint8Array(dataUri.substring(base64Idx))
  }
